...
Fecha analizar_fecha(char *line) {
    return fecha_crear(0, 0, 0);
}

void cargar_dataset(char *archivo, TablaHash *tabla) {
    FILE *fp;
    size_t len = 0;

    assert((fp = fopen(archivo, "r, ccs=UTF-8")));

    // List list = list_crear();
    char *line;
    char *token;
    char *argv[DATASET_TAMANIO];
    int argc = 0, confirmados, descartados, en_estudio, notificaciones;
    
    while ((line = getline(&line, &len, stdin)), line != NULL){
        token = strtok(line, ",");
        argc = 0;
        ...
        Registro nuevo = registro_crear(analizar_fecha(argv[0]), argv[1], argv[2], 0, 0, 0, 0);

        sscanf(argv[3], L"%d", &nuevo->confirmados);;
        sscanf(argv[3], L"%d", &nuevo->descartados);;
        sscanf(argv[3], L"%d", &nuevo->en_estudio);;
        sscanf(argv[3], L"%d", &nuevo->notificaciones);;
