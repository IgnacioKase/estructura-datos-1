...
void cargar_dataset(const char *archivo, TablaHash *tabla) {
    ...
    char *fecha_ignorar, *fecha, *departamento, *localidad;
    ...
    while (fscanf(fp, "%d-%d-%dT%s,%s,%s,%s,%s,%s,%s", anio, mes, dia, fecha_ignorar, departamento, localidad, confirmados, descartados, en_estudio, notificaciones)) {
        Fecha fecha_nuevo = fecha_crear(dia, mes, anio);
        nuevo = registro_crear(analizar_fecha(argv[0]), argv[1], argv[2], 0, 0, 0, 0);

        sscanf(argv[3], "%d", &nuevo->confirmados);
        sscanf(argv[4], "%d", &nuevo->descartados);
        sscanf(argv[5], "%d", &nuevo->en_estudio);
        sscanf(argv[6], "%d", &nuevo->notificaciones);
