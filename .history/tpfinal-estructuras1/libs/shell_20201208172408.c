...
void cargar_dataset(const char *archivo, TablaHash *tabla) {
    ...
    fscanf(fp, "%s", fecha_ignorar);
    
    while (fscanf(fp, "%d-%d-%dT%s,%s,%s,%d,%d,%d,%d", &anio, &mes, &dia, fecha_ignorar, departamento, localidad, &confirmados, &descartados, &en_estudio, &notificaciones)) {
