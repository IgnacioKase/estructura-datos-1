...
char* registro_obtener_clave(Registro registro) {
    char *s = mallocc(sizeof(char*) * (strlen(registro->departamento) + strlen(registro->localidad) + 2));
...
void registro_imprimir(Registro registro) {
    printf("%s,%s,%s,%d,%d,%d,%d\n", registro->fecha, registro->departamento, registro->localidad, registro->confirmados, registro->descartados, registro->en_estudio, registro->notificaciones);
}