...
void agregar_registro(const char *argv[6], TablaHash tabla) {
    ...
    if(!list_vacia(lista)) {
        ...
        if(!list_vacia(nodo)) {
            ...
            registro->descartados = nuevo->confirmados;
            registro->en_estudio = nuevo->confirmados;
            registro->notificaciones = nuevo->confirmados;
            ...
            lista = list_agregar_ordenado(lista, nuevo, fecha_es_menor);
