...
Fecha analizar_fecha(char line*) {
    return Fecha fecha_nuevo = fecha_crear(0, 0, 0);
}
...
void cargar_dataset(char *archivo, TablaHash *tabla) {
    ...
    while ((line = getline(&line, &len, stdin)), line != NULL){
        ...
        Fecha fecha_nuevo = fecha_crear(0, 0, 0);
        confirmados = argv[3];
        descartados = argv[4];
        en_estudio = argv[5];
        notificaciones = argv[6];
        Registro nuevo = registro_crear(fecha_nuevo, argv[1], argv[2], confirmados, descartados, en_estudio, notificaciones);
    }
    free(line);
    fclose(fp);
}
