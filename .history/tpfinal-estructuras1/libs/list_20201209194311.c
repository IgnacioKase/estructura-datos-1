...
/*
    Agrega un elemento de forma ordenada de menor a mayor a partir de
    la función 'menor'.
*/
List list_agregar_ordenado(List lista, void *dato, FuncionMenor menor) {
  ...
  if (lista == NULL) {
    ...
    while (!menor(iterador->dato, dato)) {
      if (iterador->sig == lista) {
      nuevo->sig = lista;
      nuevo->ant = iterador;
      iterador->sig = nuevo;
      lista->ant = nuevo;
