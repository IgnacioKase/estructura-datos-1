...
void buscar_pico(TablaHash tabla, const char *departamento, const char *localidad) {
    ...
    for (nodo = lista; nodo->sig != lista; nodo = nodo->sig) {
        if(lista->sig != NULL) {
            if(lista->sig->dato != NULL) {
                temp = ((Registro) nodo->dato)->confirmados - ((Registro) lista->dato)->confirmados;
                printf("%d\t%d", ((Registro) nodo->dato)->confirmados,((Registro) lista->dato)->confirmados):
                printf(temp);
                if (temp > max) {
                    max = temp;
    ...
    printf("El pico para la localidad de: %s, fue en la fecha: %d-%d-%d con un total de %lu\n", clave, fecha->anio, fecha->mes, fecha->dia, max);
