...
void graficar(TablaHash tabla, const char *fecha_inicio_char, const char *fecha_fin_char, const char *departamento, const char *localidad) {
    ...
    sprintf(titulo, "set title 'Gráfica casos confirmados y acumulados [\"%02d%c%02d\":\"%02d%c%02d\"] en %s'",
            fecha_inicio->mes, 45, fecha_inicio->dia,
            fecha_fin->mes, 45, fecha_fin->dia, clave);
    ...
    char * comandos_gnuplot[] = {
        ...
        "set output 'grafica-confirmados.html'",
        "plot 'confirmados.temp' using 1:2 lt rgb 'red' w l title 'Confirmados'",
        "set output 'grafica-acumulados.html'",
        "plot 'acumulados.temp' using 1:2 lt rgb 'green' w l title 'Acumulados'"
    };
    ...
    for (iterador = inicio; iterador != fin; iterador = iterador->sig) {
        ...
        fprintf(archivo_confirmados, "%02d%c%02d,%d\n", fecha->mes, 45, fecha->dia, confirmados); //Write the data to a temporary file
        // fprintf(archivo_acumulados, "%lf\t %lf\t \n", (double)acumulados, (double)acumulados); //Write the data to a temporary file
        fprintf(archivo_acumulados, "%02d%c%02d,%d\n", fecha->mes, 45, fecha->dia, acumulados); //Write the data to a temporary file
