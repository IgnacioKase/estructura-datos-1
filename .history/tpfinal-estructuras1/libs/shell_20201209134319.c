...
void buscar_pico(TablaHash tabla, const char *departamento, const char *localidad) {
    ...
    for (nodo = lista; nodo->sig != lista; nodo = nodo->sig) {
        if(lista->sig != NULL) {
            if(lista->sig->dato != NULL) {
                temp = ((Registro) lista->dato)->confirmados - ((Registro) nodo->dato)->confirmados;
                if (temp > max) {
                    max = temp;
    ...
    printf("El pico para la localidad de: %s, fue en la fecha: %d-%d-%d con un total de %d", clave, registro->fecha->anio, registro->fecha->mes, registro->fecha->dia, max);
