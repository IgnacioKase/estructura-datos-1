...
void graficar(TablaHash tabla, const char *fecha_inicio_char, const char *fecha_fin_char, const char *departamento, const char *localidad) {
    ...
    int num_commands = 2;
    char * comandos_gnuplot[num_commands] = {"set terminal canvas set title \"Evolución de los casos confirmados\"",
    ...
    for (int i = 0; i < num_commands; i++) {
        fprintf(gnuplotPipe, "%s \n", comandos_gnuplot[i]); //Send commands to gnuplot one by one.
    }
