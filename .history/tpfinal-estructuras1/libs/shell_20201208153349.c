...
void cargar_dataset(char *archivo, TablaHash *tabla) {
    ...
    while ((line = getline(&line, &len, stdin)), line != NULL){
        ...
        confirmados = argv[3]swscanf(pre_edad, L"%d", &p->edad);;
        descartados = argv[4];
        en_estudio = argv[5];
        notificaciones = argv[6];

        Registro nuevo = registro_crear(analizar_fecha(argv[0]), argv[1], argv[2], confirmados, descartados, en_estudio, notificaciones);
    }
    free(line);
    fclose(fp);
}
