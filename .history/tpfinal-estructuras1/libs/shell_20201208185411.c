...
void merge_sort_adaptado(void *lista)
...
void cargar_dataset(const char *archivo, TablaHash *tabla) {
    ...
    for (unsigned idx = 0; idx < tabla->capacidad; ++idx) {
        if(tabla->tabla[idx].dato != NULL) {
            merge_sort(tabla->tabla[idx].dato, registro_menor);
        }
        if(tabla->tabla[idx].areaRebalse != NULL){
            list_recorrer_extra(tabla->tabla[idx].areaRebalse, merge_sort, registro_menor, LIST_RECORRIDO_HACIA_ADELANTE);
        }
    }
