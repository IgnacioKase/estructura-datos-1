...
void tiempo_duplicacion(TablaHash tabla, const char *fecha_char, const char *departamento, const char *localidad) {
    ...
    Registro temp;
    double objetivo = busqueda->confirmados * 2, aux;
    ...
    for (iterador = nodo->ant; iterador->ant != lista->ant; iterador = iterador->ant) {
        ...
        if (temp->confirmados == 0)
            continue;
        aux = (double) busqueda->confirmados / (double) temp->confirmados;
        if(fabs(2-aux) < fabs(2-mejor_candidato)) {
            mejor_candidato = aux;
            fecha_mejor_candidato = temp->fecha;
        }
    }

    printf("El tiempo de duplicación para la localidad de: %s, fue de %d, en la fecha: %d-%d-%d con un ratio de %.2f\n", 
            clave,
