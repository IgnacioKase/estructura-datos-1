...
void graficar(TablaHash tabla, const char *fecha_inicio_char, const char *fecha_fin_char, const char *departamento, const char *localidad) {
    ...
    List iterador;
    List inicio = (List) list_buscar(lista, inicio, registro_igual_por_fecha);
    List fin = (List) list_buscar(lista, fin, registro_igual_por_fecha);
    registro_destruir(busqueda_inicio);
    registro_destruir(busqueda_fin);

    int confirmados, acumulados, confirmados_temp = 0;

    for (iterador = inicio; iterador->sig != fin->sig; iterador = iterador->sig) {
        acumulados = ((Registro) iterador->dato)->confirmados;
        confirmados = acumulados - confirmados_temp;
        printf("Casos confirmados: %d, Casos acumulados: %d\n", confirmados, acumulados);
        confirmados_temp = confirmados;
    }
