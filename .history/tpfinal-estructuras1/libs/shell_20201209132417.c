...
void cargar_dataset(const char *archivo, TablaHash tabla) {
    ...
    while (fscanf(fp, "%d-%d-%dT%39[^,],%39[^,],%39[^,],%d,%d,%d,%d\n", &anio, &mes, &dia, fecha_ignorar, departamento, localidad, &confirmados, &descartados, &en_estudio, &notificaciones) != EOF) {
        ...
        if(lista != NULL) {
            list_agregar_ordenado(&lista, (void *) nuevo, registro_menor);
        } else {
            lista = list_crear();
...
void agregar_registro(const char *argv[6], TablaHash tabla) {
    ...
    Registro nuevo = registro_crear(fecha, departamento, localidad, atoi(argv[3]), atoi(argv[4]), atoi(argv[5]), 0);
    ...
    if(!list_vacia(lista)) {
        ...
        if(!list_vacia(nodo)) {
            ...
            if(strlen(nuevo->departamento) > 0)
                free(nuevo->departamento);
            if(strlen(nuevo->localidad) > 0)
                free(nuevo->localidad);
            if(strlen(nuevo->clave) > 0 && nuevo->clave != NULL)
                free(nuevo->clave);
            free(nuevo);
    ...
    if(strlen(busqueda->departamento) > 0)
        free(busqueda->departamento);
    if(strlen(busqueda->localidad) > 0)
        free(busqueda->localidad);
    if(strlen(busqueda->clave) > 0 && nuevo->clave != NULL)
        free(busqueda->clave);
    free(nuevo);
    free(clave);
}
