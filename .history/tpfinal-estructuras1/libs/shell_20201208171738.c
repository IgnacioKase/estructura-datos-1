...
void cargar_dataset(const char *archivo, TablaHash *tabla) {
    ...
    char *argv[DATASET_TAMANIO][300];
    Registro nuevo;

    // Ignoramos los titulos de las columnas
    fscanf(fp, "%s,%s,%s,%s,%s,%s,%s", argv[0], argv[1], argv[2], argv[3], argv[4], argv[5], argv[6]);
    
    while (fscanf(fp, "%s,%s,%s,%s,%s,%s,%s", argv[0], argv[1], argv[2], argv[3], argv[4], argv[5], argv[6])){
        token = strtok(line, ",");
        argc = 0;
