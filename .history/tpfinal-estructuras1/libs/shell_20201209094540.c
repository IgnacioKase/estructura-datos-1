...
void eliminar_registro(TablaHash tabla, const char *fecha_char, const char *departamento, const char *localidad) {
    ...
    List nodo = list_buscar(lista, busqueda)
...
/* Esta función no se encuentra en list.h porque es especifica para
   el tipo de dato Registro */
Registro list_buscar_por_fecha(List lista, Fecha fecha) {
    Nodo *nodo;

    for (nodo = lista; nodo->sig != lista; nodo = nodo->sig){
        if(fecha_es_igual(fecha, nodo->))
    }
}
...
void casos_acumulados(TablaHash tabla, const char *fecha_char, const char *departamento, const char *localidad) {
    ...
    Registro ultimo = (Registro) lista->ant->dato;
    printf("")
    fecha_destruir(fecha);
    free(clave);
}
