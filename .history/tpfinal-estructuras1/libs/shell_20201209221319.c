...
void graficar(TablaHash tabla, const char *fecha_inicio_char, const char *fecha_fin_char, const char *departamento, const char *localidad) {
    ...
    int num_commands = 2;
    char * comandos_gnuplot[] = {"set terminal canvas set title \"Evolución de los casos confirmados\"",
	                           "plot 'confirmados.temp' with lines, 'acumulados.temp' with lines"};
    ...
    for (iterador = inicio; iterador != fin; iterador = iterador->sig) {
        ...
        fprintf(archivo_confirmados, "%d\t %d\t \n", confirmados, confirmados); //Write the data to a temporary file
        // fprintf(archivo_confirmados, "%d\t %02d%c%02d%c%02d\t \n", confirmados, fecha->anio, 45, fecha->mes, 45, fecha->dia); //Write the data to a temporary file
        fprintf(archivo_acumulados, "%d\t %d\t \n", acumulados, acumulados); //Write the data to a temporary file
        // fprintf(archivo_acumulados, "%d\t %02d%c%02d%c%02d\t \n", acumulados, fecha->anio, 45, fecha->mes, 45, fecha->dia); //Write the data to a temporary file
