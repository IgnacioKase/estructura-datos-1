...
void registro_imprimir(Registro registro) {
    printf("%d-%d-%d,%s,%s,%d,%d,%d,%d\n", (Fecha registro->fecha)->anio, (registro->fecha)->mes, (registro->fecha)->dia, registro->departamento, registro->localidad, registro->confirmados, registro->descartados, registro->en_estudio, registro->notificaciones);
}