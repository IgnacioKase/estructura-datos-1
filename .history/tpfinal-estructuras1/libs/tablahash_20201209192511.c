...
TablaHash _tablahash_crear(int size, FuncionHash fun, FuncionIgualdad igualdad, DestruirExterno destruir, DestruirInterno destruir_interno)
{
    TablaHash d;
    int i;
    ...
    d->igualdad = igualdad;
    d->destruir = destruir;
    d->destruir_interno = destruir_interno;
...
{
    return _tablahash_crear(INITIAL_SIZE, fun, igualdad, destruir, destruir_interno);
}
...
void tablahash_destruir(TablaHash d)
{
    ...
    for(i = 0; i < d->size; i++) {
        for(e = d->table[i]; e != 0; e = next) {
            ...
            // la clave la libera el destruir
            d->destruir(e->value, d->destruir_interno);
...
static void _tablahash_crecer(TablaHash d)
{
    ...
    d2 = _tablahash_crear(d->size * GROWTH_FACTOR, d->hash, d->igualdad, d->destruir, d->destruir_interno);
...
void tablahash_insertar(TablaHash d, void *key, void *value)
{
    struct elt *e;
    unsigned long h;
...
void * tablahash_buscar(TablaHash d, void *key)
{
    struct elt *e;
...
void tablahash_eliminar(TablaHash d, void *key)
{
