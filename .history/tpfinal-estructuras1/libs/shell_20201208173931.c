...
void cargar_dataset(const char *archivo, TablaHash *tabla) {
    ...
    while (fscanf(fp, "%d-%d-%dT%39[^,],%39[^,],%39[^,],%d,%d,%d,%d\n", &anio, &mes, &dia, fecha_ignorar, departamento, localidad, &confirmados, &descartados, &en_estudio, &notificaciones)) {
        printf("%s\n", fecha_ignorar);
        Fecha fecha_nuevo = fecha_crear(dia, mes, anio);

        Registro nuevo = registro_crear(fecha_nuevo, departamento, localidad, confirmados, descartados, en_estudio, notificaciones);
