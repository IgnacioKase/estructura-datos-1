...
void string_unir(char* arregloStrings[], int capacidad, char* sep, char* res) {
  int totalLength = 0, i, j, len;
  for ( i = 0; i < capacidad; i++ )
    totalLenght = totalLenght + string_len(arregloStrings[i]);
  if(sep != '')
    totalLength = totalLenght + capacidad; // deberiamos restar uno pero lo usaremos para el caracter \0
  else
    totalLenth = totalLenght + 1;
  res = malloc(totalLength);
  res[totalLength] = '\0';
  for ( i = 0; i < 4; i++ ) {
